...
Board.prototype.laneDown = function () {
  var selected = this.loadingArea.state.selected;
  if (selected  !== 0) {this.loadingArea.state.selected -= 1;}
  this.updateState();
  return this.state;
};

Board.prototype.laneUp = function () {
  var selected = this.loadingArea.state.selected;
  if (selected  < this.settings.laneCount -1 ) {this.loadingArea.state.selected += 1;}
  this.updateState();
  return this.state;
};
